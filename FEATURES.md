...
- **Elder Guardian Appearance Suppression** - Disable the Elder Guardian Mining Fatigue appearance effect, with a separate client toggle to keep or silence the normal sound
...
- **Night Vision Fade** - Smooth night vision transitions instead of vanilla blinking, including permanent/infinite-duration night vision
...
- **Soul Fire Visuals** - When burning from Soul Fire / Soul Campfire, render the on-screen fire overlay (and burning entity flames) using the blue soul-fire sprites for the duration of the burn; lit candles placed on soul sand or soul soil also use blue soul-fire flame particles, with a client toggle for the candle flame effect
- **Suppress Experimental Settings Warning** - Suppress experimental settings warnings when creating/loading worlds
- **Usage Ticker** - Visual item usage progress for held items with configurable positioning
...
- **Snowball Rework** - Snowballs deal damage and apply freezing effects with special Nether entity damage multiplier, without forcing vanilla powder-snow movement behavior onto hit entities
- **Source Dependent Invulnerability Frames** - Give different damage-source families independent victim i-frame timers so overlapping sources can damage normally without fully disabling cooldowns (with configurable blacklist)
...
- **Bioluminescence Potion** - Brewable Bioluminescence effect (Recipe: Glowing Potion + Glow Goo; Glowstone upgrades it to Bioluminescence II; long variant from Long Glowing Potion + Glow Goo) with 4:00 / 2:00 II / 8:00 durations, splash and lingering variants, tipped arrows, and recipe-book / JEI support
- **Displacement Potion** - Random teleportation within 100-block radius (Recipe: Awkward Potion + Ender Eye)
- **Glowing Potion** - Brewable glowing effect (Recipe: Awkward Potion + Glow Berries, extendable with Redstone) with 4:00 / 8:00 durations, splash and lingering variants, tipped arrows, and recipe-book / JEI support
...
- **Chat Mentions** - Username highlighting and ping sounds with @username or plain username detection, case-insensitive with word boundaries, plus a live suggestion list, Tab completion for online usernames while typing `@name`, and local pre-send input highlighting once a full username is typed in chat
...
- **Dismount Entities** - Crouch+right-click an occupied vehicle or its mounted rider to eject passengers; ejection is forward-biased, collision-safe, and plays a saddle-equip sound
...
- **Quick Harvesting** - Right-click harvest that behaves like breaking with your current tool (enchant/loot-context compatible):
  ...
  - **Drops** - Quick Harvesting item drops home toward the user for 1 second by default
...
- **Villagers Follow Emeralds** - Villagers follow players holding emeralds/emerald blocks with configurable speed and particle effects

## Mob Modifications
- **Cave Spider Spawning** - Cave spiders have a configurable chance to replace spiders in caves
- **Depth Scaling Enemies** - Depth-based scaling for hostile mob equipment spawns; chestplates are the most common piece, surface-tier weights favor leather, and the gear pool now includes copper armor and zombie copper tools alongside the other depth-tier materials (configurable). Each tier (surface/mid/deep) can be toggled independently; surface tier is disabled by default.
...
- **Improved Phantoms** - Multiple enhancements:
  ...
  - **Automatic Drop** - Phantoms can automatically drop carried players after a configurable carry duration, which defaults to 3 seconds
  - **Drop On Hit** - Hitting the phantom that is carrying you can force it to drop you immediately
  - **Hit Dismount Slow Falling** - If you hit the phantom during a carry, the carry ending grants 2 seconds of Slow Falling I
  - **Carry Counterplay** - Allows damaging the carrying phantom while being carried (server-side swing detection)
...
- **Bottle o' Cloud** - Obtain a Bottle o' Cloud by right-clicking a Glass Bottle at cloud height (Y ≥ 192); place it to create Cloud Blocks, pick those blocks back up again with an empty bottle, and use the clouds as a soft landing surface that reduces fall damage to half by default with a configurable multiplier
- **Glow Goo** - Throwable Glow Goo splats into Goo Splatter, a bright, nearly-flat light source that can stick to floors, walls, and ceilings, sit in water while waterlogged, drop back into Glow Goo when broken, and also serve as the brewing reagent for Bioluminescence potions
- **Brushing XP Rewards** - XP from finishing brushable blocks with configurable min/max values
- **Candle Bundles (Mixed Candles)** - Allows bundling different candle items into the same multi-candle block; each candle preserves its own dye/tint visuals and drops back correctly, mixed bundles orient toward the placer so the smallest candle faces you, and the flame/smoke particles follow that stored orientation (configurable)
...
- **Double Door Opening** - Double doors open together with redstone mirroring support, crouch for single door, chain trapdoors (up to 5)
- **Enhanced Slab Behavior (Mixed Slabs + Slab QoL)** - A bundle of slab improvements (configurable):
  ...
  - **Material Behavior Inheritance** - Generated wooden slab-family variants inherit fuel and fire behavior from their source slabs, and generated copper slab-family variants support oxidation, waxing, unwaxing, scraping, and the matching vanilla sounds/particles
...
- **Improved Recovery Compass** - Keep on death, show distance/coords, right-click to toggle display, and launch a short-lived soul trail toward your last death location on shift+right-click
...
- **Pistons Move Tile Entities** - Lets pistons push and pull supported block entities such as chests and furnaces while preserving their stored block-entity data (configurable)
- **Responsive Shields** - Remove shield blocking delay with configurable raise time (0 = instant blocking)
- **Return to Killer** - Airborne kills tag nearby drops/XP to home toward the killer (15s); items ramp up faster at long range, ease in near the killer, and still keep the 5 m/s speed cap
