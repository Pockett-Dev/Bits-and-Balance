...
public final class TemporaryCloudFadeRenderer {
    ...
    public static void queue(PoseStack.Pose currentPose, BlockPos blockPos, int alpha, int packedLight) {
        if (currentPose == null || blockPos == null || alpha <= 0) {
            return;
        }
    ...
    public static void renderAllDeferred() {
        ...
        try {
            ...
            for (QueuedCloud cloud : QUEUE) {
                poseStack.pushPose();
                poseStack.last().pose().set(cloud.pose);
                poseStack.last().normal().set(cloud.normal);
                bitsandbalance$renderCloud(poseStack.last(), vc, level, cloud.blockPos, cloud.alpha, cloud.packedLight, u0, u1, v0, v1);
                poseStack.popPose();
            }
    ...
    private static int resolveFacePackedLight(ClientLevel level, BlockPos pos, Direction direction, int fallbackPackedLight) {
        ...
        int block = level.getBrightness(LightLayer.BLOCK, samplePos);
        int sky = level.getBrightness(LightLayer.SKY, samplePos);
        return (block << 4) | (sky << 20);
    }

    private static boolean classExists(String className) {
        try {
            Class.forName(className, false, TemporaryCloudFadeRenderer.class.getClassLoader());
            return true;
    ...
    private TemporaryCloudFadeRenderer() {
    }
    private static TextureAtlasSprite bitsandbalance$getCloudSprite(Minecraft minecraft) {
        ...
        if (atlasManager == null) {
            ...
            throw new IllegalStateException("Failed to resolve block atlas for temporary cloud fade rendering");
        }
