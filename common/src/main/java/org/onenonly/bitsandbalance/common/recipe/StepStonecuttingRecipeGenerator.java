...
public final class StepStonecuttingRecipeGenerator {
    ...
    public static Map<String, String> generate(RecipeManager recipeManager, RegistryAccess registryAccess) {
        ...
        recipes.putAll(generateMirroredStonecuttingRecipes(recipeManager, registryAccess));
        recipes.putAll(generateDirectStonecuttingRecipes());
        return recipes;
    }
    ...
    public static Map<String, String> generateMirroredStonecuttingRecipes(RecipeManager recipeManager, RegistryAccess registryAccess) {
        ...
        addRecipesFromExistingStonecutting(recipeManager, registryAccess, recipes, seenSimpleConversions);
        return recipes;
    }
    ...
    public static Map<String, String> generateDirectStonecuttingRecipes() {
        Map<String, String> recipes = new LinkedHashMap<>();
        Set<String> seenSimpleConversions = new HashSet<>();
        addWoodBaseRecipes(recipes, seenSimpleConversions);
        return recipes;
    }

    private static void addRecipesFromExistingStonecutting(RecipeManager recipeManager, RegistryAccess registryAccess, Map<String, String> out, Set<String> seenSimpleConversions) {
        try {
            for (RecipeHolder<?> holder : recipeManager.getRecipes()) {
                if (!(holder.value() instanceof StonecutterRecipe stonecutting)) continue;

                Identifier recipeId = holder.id().identifier();
                if (shouldSkipMirroredRecipe(recipeId)) continue;

                ItemStack output = RecipeCompat.assemble(stonecutting, new SingleRecipeInput(ItemStack.EMPTY), registryAccess);
    ...
    private static boolean shouldSkipMirroredRecipe(Identifier recipeId) {
        return recipeId == null || recipeId.getPath().contains("_wood_sc_");
    }

    private static void addWoodBaseRecipes(Map<String, String> out, Set<String> seenSimpleConversions) {
        for (Item inputItem : BuiltInRegistries.ITEM) {
            ...
            if (path.endsWith("_planks")) {
                ...
                addWoodBaseRecipe(out, inputId, ns, base + "_slab", 4, "wood_planks_step", true, seenSimpleConversions);
            } else if (path.endsWith("_log")) {
                addWoodTypeStepRecipes(out, inputId, ns, path, "_log", seenSimpleConversions);
            } else if (path.endsWith("_wood")) {
                addWoodTypeStepRecipes(out, inputId, ns, path, "_wood", seenSimpleConversions);
            } else if (path.endsWith("_stem")) {
                addWoodTypeStepRecipes(out, inputId, ns, path, "_stem", seenSimpleConversions);
            } else if (path.endsWith("_hyphae")) {
                addWoodTypeStepRecipes(out, inputId, ns, path, "_hyphae", seenSimpleConversions);
            }
        }
    }
    ...
    private static void addWoodTypeStepRecipes(Map<String, String> out,
        ...
        addWoodBaseRecipe(out, inputId, namespace, familyBase + "_slab", 16, sortKeyPrefix, true, seenSimpleConversions);
    }
