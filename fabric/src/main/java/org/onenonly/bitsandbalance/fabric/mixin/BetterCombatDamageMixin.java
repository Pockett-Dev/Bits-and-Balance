...
public abstract class BetterCombatDamageMixin {
    ...
    private void bitsandbalance$modifyPhantomDamage(Args args) {
        ...
        if (FabricMobsConfig.enablePhantomDoubleDamage && MountedPhantomAttackContext.matches(phantom)) {
        ...
        final Player attackingPlayer = player;

        if (attackingPlayer.isPassenger() && attackingPlayer.getVehicle() == phantom) {
            if (attackingPlayer instanceof PhantomPickupTracker tracker) {
                tracker.bitsandbalance$setPhantomHitDuringCarry(true);

                if (FabricMobsConfig.enablePhantomDropOnHit) {
                    ((PhantomDismountTracker) (Object) phantom).bitsandbalance$recordDismount(attackingPlayer.getUUID(), phantom.level().getGameTime());

                    var server = phantom.level().getServer();
                    if (server == null) {
                        ImprovedPhantomCarryHelper.forceDrop(attackingPlayer, phantom, tracker, true);
                    } else {
                        server.execute(() -> ImprovedPhantomCarryHelper.forceDrop(attackingPlayer, phantom, tracker, true));
                    }
                }
            }

            if (FabricMobsConfig.enablePhantomDoubleDamage) {
                args.set(2, amount * 2.0f);
            }
        }
    }
