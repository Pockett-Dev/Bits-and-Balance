...
public final class FabricBioluminescenceSync {
        ...
        EntityTrackingEvents.START_TRACKING.register((trackedEntity, player) -> {
            ...
            if (remainingTicks > 0) {
                ServerPlayNetworking.send(
                        player,
                        new BioluminescenceSyncPayload(
                                living.getId(),
                                remainingTicks,
                                BioluminescenceLightManager.getBioluminescenceAmplifier(living)
                        )
                );
            }
        });
    ...
    public static void initClient() {
        ClientPlayNetworking.registerGlobalReceiver(BioluminescenceSyncPayload.TYPE, (payload, context) -> {
    ...
    public static void syncIfServer(LivingEntity entity, int remainingTicks) {
        ...
        BioluminescenceSyncPayload payload = new BioluminescenceSyncPayload(
                entity.getId(),
                remainingTicks,
                BioluminescenceLightManager.getBioluminescenceAmplifier(entity)
        );
