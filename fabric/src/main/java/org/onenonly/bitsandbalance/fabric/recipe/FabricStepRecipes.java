...
public final class FabricStepRecipes {
    ...
    private static boolean syncDatapack(Path worldRoot, PackRepository packRepository, RecipeManager recipeManager, RegistryAccess registryAccess, boolean reloadAfterSync) throws IOException {
        ...
        String inputFingerprint = DynamicRecipeInputFingerprints.buildStepFingerprint(
            recipeManager,
            registryAccess,
        ...
        Map<String, String> recipes = new LinkedHashMap<>(StepStonecuttingRecipeGenerator.generate(
            recipeManager,
            registryAccess,
            FabricBuildingConfig.enhancedSlabsVerticalSlabs
        ));
        recipes.putAll(EnhancedSlabCraftingRecipeGenerator.generateStepAndRecoveryRecipes(FabricBuildingConfig.enhancedSlabsVerticalSlabs));
        if (recipes.isEmpty()) {
