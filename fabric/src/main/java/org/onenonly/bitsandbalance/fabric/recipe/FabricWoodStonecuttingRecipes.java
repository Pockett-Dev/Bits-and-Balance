...
public final class FabricWoodStonecuttingRecipes {
    ...
    private static boolean syncDatapack(Path worldRoot, PackRepository packRepository, boolean reloadAfterSync) throws IOException {
        ...
        if (DynamicRecipeDatapackSync.matchesInputFingerprint(datapackPath, inputFingerprint)) {
            ...
            return FabricGeneratedWorldDataPacks.enableGeneratedPack(packRepository, DATAPACK_NAME, false);
        }
        ...
        boolean selectionChanged = FabricGeneratedWorldDataPacks.enableGeneratedPack(packRepository, DATAPACK_NAME, result == DynamicRecipeDatapackSync.Result.WRITTEN);
        return result != DynamicRecipeDatapackSync.Result.UNCHANGED || selectionChanged;
    }
}
