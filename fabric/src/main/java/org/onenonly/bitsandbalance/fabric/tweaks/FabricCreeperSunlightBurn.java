...
public final class FabricCreeperSunlightBurn {
    ...
    private static boolean shouldBurnInSunlight(Creeper creeper, Level level) {
        int timeOfDay = (int) (bitsandbalance$getDayTime(level) % 24000L);
    ...
    private static long bitsandbalance$getDayTime(Level level) {
        if (level == null) {
            return 0L;
        }

        try {
            Method getDayTime = level.getClass().getMethod("getDayTime");
            Object value = getDayTime.invoke(level);
        ...
        try {
            Method getOverworldClockTime = level.getClass().getMethod("getOverworldClockTime");
            Object value = getOverworldClockTime.invoke(level);
            if (value instanceof Long longValue) {
                return longValue;
            }
        } catch (ReflectiveOperationException ignored) {
