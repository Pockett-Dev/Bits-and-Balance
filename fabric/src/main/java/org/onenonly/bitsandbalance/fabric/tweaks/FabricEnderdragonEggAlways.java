...
public class FabricEnderdragonEggAlways {
    ...
    public static void init() {
        ServerLivingEntityEvents.AFTER_DEATH.register((entity, damageSource) -> {
            ...
            Object dragonFight = FabricDragonFightCompat.getDragonFight(serverLevel);
            ...
            boolean isRespawnedDragon = FabricDragonFightCompat.hasPreviouslyKilledDragon(dragonFight);
        ...
        for (int xOffset = -5; xOffset <= 5; xOffset++) {
            for (int yOffset = -5; yOffset <= 10; yOffset++) {
                for (int zOffset = -5; zOffset <= 5; zOffset++) {
                    ...
                    if (serverLevel.getBlockState(checkPos).getBlock() == Blocks.DRAGON_EGG) {
                        eggPos = checkPos;
                        break;
                    }
                }
    ...
    private static BlockPos findSafeSpawnLocation(ServerLevel level, BlockPos center) {
        ...
        for (int attempt = 0; attempt < 50; attempt++) {
            ...
            int xOffset = level.getRandom().nextInt(radius * 2 + 1) - radius;
            int zOffset = level.getRandom().nextInt(radius * 2 + 1) - radius;
