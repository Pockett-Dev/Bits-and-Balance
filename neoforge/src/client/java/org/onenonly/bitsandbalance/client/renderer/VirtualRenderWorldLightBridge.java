...
public final class VirtualRenderWorldLightBridge {
    ...
    private static void bitsandbalance$emitQuadDirect(Object quad, PoseStack.Pose pose, VertexConsumer consumer, int packedLight) {
        try {
            ...
            VertexConsumer quadConsumer = consumer;
            ...
            if (materialInfoMethod != null) {
                ...
                if (materialInfo != null) {
                    ...
                    if (spriteMethod != null) {
                        ...
                        spriteForLog = sprite;
                        if (sprite != null) {
                            Method wrapMethod = SPRITE_WRAP.computeIfAbsent(sprite.getClass(), type -> {
                                try {
                                    return type.getMethod("wrap", VertexConsumer.class);
                                } catch (NoSuchMethodException e) {
                                    return null;
                                }
                            });
                            if (wrapMethod != null) {
                                Object wrapped = wrapMethod.invoke(sprite, consumer);
                                if (wrapped instanceof VertexConsumer wrappedConsumer) {
                                    quadConsumer = wrappedConsumer;
                                }
                            }
                        }
                    }
            ...
            for (int vertex = 0; vertex < 4; vertex++) {
                ...
                float rawU = Float.intBitsToFloat((int) packedUv);
                float rawV = Float.intBitsToFloat((int) (packedUv >>> 32));
                float u = rawU / 16.0f;
                float v = rawV / 16.0f;
                if (!LOGGED_SAMPLE_QUAD && vertex == 0) {
                    LOGGED_SAMPLE_QUAD = true;
                    LOGGER.warn("Sample fallback quad: direction={}, rawU={}, rawV={}, normalizedU={}, normalizedV={}, sprite={}",
                            direction, rawU, rawV, u, v, spriteForLog);
                }

                quadConsumer.addVertex(pose, position.x(), position.y(), position.z())
