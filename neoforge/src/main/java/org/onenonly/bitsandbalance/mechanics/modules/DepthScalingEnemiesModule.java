 ...
 * - Weighted armor material selection (leather, chain, copper, iron, gold)
...
public class DepthScalingEnemiesModule implements FeatureModule {
    ...
    /**
     * Main depth scaling logic - triggered when a mob spawn is finalized.
     */
    @SubscribeEvent
    public static void onFinalizeSpawn(FinalizeSpawnEvent event) {
        ...
        applyDepthScalingForSpawn(mob, level);
    }

    public static void applyDepthScalingForSpawn(Mob mob, ServerLevel level) {
        try {
    ...
    private static String selectArmorMaterial(RandomSource rand, ArmorConfig config) {
        double totalWeight = config.leatherWeight + config.chainWeight + config.copperWeight + config.ironWeight + config.goldWeight;
        ...
        if (roll < config.leatherWeight + config.chainWeight + config.copperWeight) return "copper";
        if (roll < config.leatherWeight + config.chainWeight + config.copperWeight + config.goldWeight) return "gold";
        return "iron";
    }
    ...
    private static String selectWeaponMaterial(RandomSource rand, WeaponConfig config) {
        double totalWeight = config.woodWeight + config.stoneWeight + config.copperWeight + config.goldWeight + config.ironWeight;
        if (totalWeight <= 0.0) totalWeight = 1.0;
        ...
        if (roll < config.woodWeight + config.stoneWeight + config.copperWeight) return "copper";
        if (roll < config.woodWeight + config.stoneWeight + config.copperWeight + config.goldWeight) return "gold";
        return "iron";
    }
    ...
    private static ItemStack createArmorPiece(EquipmentSlot slot, String material) {
        return switch (slot) {
            case HEAD -> switch (material) {
                ...
                case "copper" -> new ItemStack(Items.COPPER_HELMET);
            ...
            case CHEST -> switch (material) {
                ...
                case "copper" -> new ItemStack(Items.COPPER_CHESTPLATE);
            ...
            case LEGS -> switch (material) {
                ...
                case "copper" -> new ItemStack(Items.COPPER_LEGGINGS);
            ...
            case FEET -> switch (material) {
                ...
                case "copper" -> new ItemStack(Items.COPPER_BOOTS);
    ...
    private static ItemStack createWeapon(String weaponType, String material) {
        return switch (weaponType) {
            case "shovel" -> switch (material) {
                ...
                case "copper" -> new ItemStack(Items.COPPER_SHOVEL);
            ...
            case "pickaxe" -> switch (material) {
                ...
                case "copper" -> new ItemStack(Items.COPPER_PICKAXE);
            ...
            default -> switch (material) { // sword
                ...
                case "copper" -> new ItemStack(Items.COPPER_SWORD);
