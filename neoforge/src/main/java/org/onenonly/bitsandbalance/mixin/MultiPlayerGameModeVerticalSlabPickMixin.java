...
/**
 * Pick-block: ensure enhanced slab/step containers pick the targeted sub-part.
 *
 * Vanilla pick-block for blocks does not include hit location in the clone-stack call.
 * In creative this runs client-side, so we can use the current hit result to choose.
 */
...
public class MultiPlayerGameModeVerticalSlabPickMixin {
	...
	private void bitsandbalance$handleEnhancedSlabCreativePick(net.minecraft.core.BlockPos pos, boolean includeData, CallbackInfo ci) {
		try {
			...
			PickBlockHelperCompat.PickTarget pickTarget = PickBlockHelperCompat.resolve(mc.level, mc.player, hit);
	...
	private ItemStack bitsandbalance$pickTargetedEnhancedSlabPart(ItemStack original) {
		try {
			...
			PickBlockHelperCompat.PickTarget pickTarget =
				PickBlockHelperCompat.resolve(mc.level, mc.player, hit);
	...
	private boolean bitsandbalance$swapInventorySlotIntoHotbar(int inventorySlot, int hotbarSlot) {
		try {
			...
			try {
				...
				).invoke(self, containerId, inventorySlot, hotbarSlot, ContainerInput.SWAP, mc.player);
				return true;
			} catch (Throwable ignored) {
			}

			for (var method : this.getClass().getMethods()) {
				if (!method.getName().equals("handleContainerInput") && !method.getName().equals("handleInventoryMouseClick")) continue;
				Class<?>[] params = method.getParameterTypes();
				if (params.length != 5) continue;
				if (params[0] != int.class || params[1] != int.class || params[2] != int.class) continue;
				if (!params[3].isEnum()) continue;
				Object swapInput = java.lang.Enum.valueOf((Class<? extends java.lang.Enum>) params[3].asSubclass(java.lang.Enum.class), "SWAP");
				method.invoke(self, containerId, inventorySlot, hotbarSlot, swapInput, mc.player);
				return true;
			}
		} catch (Throwable ignored) {
		}
