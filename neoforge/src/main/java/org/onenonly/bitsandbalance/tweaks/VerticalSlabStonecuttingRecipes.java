...
public final class VerticalSlabStonecuttingRecipes {
    ...
    public static boolean prepareStaticDatapack() throws IOException {
        ...
        DynamicRecipeDatapackSync.Result result = DynamicRecipeDatapackSync.sync(datapackPath, STATIC_PACK_MCMETA, Map.of(), inputFingerprint);
        if (result == DynamicRecipeDatapackSync.Result.REMOVED) {
            LOGGER.info("[{}] Removed legacy vertical slab static crafting datapack entries", BitsAndBalance.MODID);
        } else {
            LOGGER.info("[{}] No vertical slab static crafting datapack entries to generate", BitsAndBalance.MODID);
        }
